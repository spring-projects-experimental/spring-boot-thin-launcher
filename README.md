...
| `thin.library` | `org.springframework.boot.experimental:spring-boot-thin-launcher:1.0.16.BUILD-SNAPSHOT` | A locator for the launcher library. Can be Maven coordinates (with optional `maven://` prefix), or a file (with optional `file://` prefix). |
...
$ java -jar myapp.jar --thin.library=org.springframework.boot.experimental:spring-boot-thin-tools-converter:1.0.16.BUILD-SNAPSHOT

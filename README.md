...
buildscript {
	ext {
		...
		wrapperVersion = '1.0.14.BUILD-SNAPSHOT'
	}
	repositories {
		mavenLocal()
		mavenCentral()
...
| `thin.library` | `org.springframework.boot.experimental:spring-boot-thin-launcher:1.0.14.BUILD-SNAPSHOT` | A locator for the launcher library. Can be Maven coordinates (with optional `maven://` prefix), or a file (with optional `file://` prefix). |
...
$ java -jar myapp.jar --thin.library=org.springframework.boot.experimental:spring-boot-thin-tools-converter:1.0.14.BUILD-SNAPSHOT

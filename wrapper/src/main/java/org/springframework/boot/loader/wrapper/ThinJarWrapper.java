...
public class ThinJarWrapper {
	...
	private static final String DEFAULT_LIBRARY = "org.springframework.boot.experimental:spring-boot-thin-launcher:jar:exec:1.0.32.BUILD-SNAPSHOT";
	...
	String download() {
		...
		if (!target.exists()) {
			...
			if (!result) {
				...
				repo = repo != null ? repo : file.contains("SNAPSHOT") ? "https://repo.spring.io/snapshot" : "https://repo1.maven.org/maven2";
				if (repo.endsWith("/")) {
